...
class SweepCondition:
    ...
    def __init__(
        ...
        render_override: dict[str, Any] | None = None,
        render_variant_name: str | None = None,
    ) -> None:
        ...
        self.render_variant_name = render_variant_name
        base_label = f"{vae_cfg.name}__{channel_strategy}__{detection_method}"
        if render_variant_name:
            self.label = f"{vae_cfg.name}__{render_variant_name}__{detection_method}"
        else:
...
class SweepExecutor:
    ...
    Conditions are generated by taking the Cartesian product of:
      ...
      * sweep_strategies (channel strategies; defaults to [config.render.channel_strategy])
        OR sweep_render_variants (render variant dicts; takes priority over sweep_strategies)
      * sweep_detectors (detection methods; defaults to [config.note_detection.method])

    When ``sweep_render_variants`` is provided the product axis becomes
    VAE x render_variant x detector instead of VAE x channel_strategy x detector.
    Each render variant dict must contain a ``name`` key plus any combination of
    ...
    Args:
        ...
        sweep_strategies: Optional list of channel strategy names to sweep.
            ...
            Ignored when sweep_render_variants is provided.
        sweep_detectors: Optional list of detection method names to sweep.
            If None, only the method in the base config is used.
        sweep_render_variants: Optional list of render variant dicts for Exp 2
            (resolution / orientation study).  Each dict must have a ``name``
            key and may override any field of RenderConfig and/or
            DataConfig.target_resolution.  When provided, this takes
            priority over sweep_strategies for the channel-axis.
    ...
    def __init__(
        ...
        sweep_render_variants: list[dict[str, Any]] | None = None,
        extra_stages: list[PipelineStage] | None = None,
    ) -> None:
        self.config = config
        self._sweep_render_variants = sweep_render_variants
        if sweep_render_variants is not None:
            # render variant mode: strategies are embedded in each variant dict
            self._sweep_strategies = None
        else:
    ...
    def conditions(self) -> list[SweepCondition]:
        ...
        When ``sweep_render_variants`` was supplied to the constructor the grid
        is VAE x render_variant x detector.  Otherwise it is the standard
        VAE x channel_strategy x detector.

        Returns:
            List of SweepCondition objects, one per combination.
        """
        ...
        if self._sweep_render_variants is not None:
            # Render-variant mode (Exp 2: resolution / orientation study)
            combos_rv = list(
                itertools.product(vaes, self._sweep_render_variants, self._sweep_detectors)
            )
            return [
                SweepCondition(
                    vae_cfg=vae_cfg,
                    channel_strategy=variant.get("channel_strategy", self.config.render.channel_strategy),
                    detection_method=detector,
                    detection_params=dict(self.config.note_detection.params),
                    render_override=variant,
                    render_variant_name=variant.get("name"),
                )
                for vae_cfg, variant, detector in combos_rv
            ]

        # Standard mode: VAE x channel_strategy x detector
        assert self._sweep_strategies is not None
        combos = list(
    ...
    def _make_condition_config(
        ...
        Creates a copy of the base config with:
          ...
          * render.pitch_axis / render fields overridden when render_override present
          * data.target_resolution overridden when render_override contains target_resolution
        ...
        """
        base = self.config
        override = condition.render_override or {}

        # Build RenderConfig — apply any render_override fields
        new_render = RenderConfig(
            channel_strategy=condition.channel_strategy,
            pitch_axis=override.get("pitch_axis", base.render.pitch_axis),
            normalize_range=tuple(override["normalize_range"])  # type: ignore[arg-type]
            if "normalize_range" in override
            else base.render.normalize_range,
            resize_method=override.get("resize_method", base.render.resize_method),
        )

        # Build DataConfig — apply target_resolution override when present
        if "target_resolution" in override:
            new_data = DataConfig(
                dataset=base.data.dataset,
                instruments=list(base.data.instruments),
                bars_per_instrument=base.data.bars_per_instrument,
                min_notes_per_bar=base.data.min_notes_per_bar,
                time_steps=base.data.time_steps,
                target_resolution=tuple(override["target_resolution"]),  # type: ignore[arg-type]
                max_files=base.data.max_files,
                max_bars_total=base.data.max_bars_total,
                subset=base.data.subset,
                pipeline_chunk_size=base.data.pipeline_chunk_size,
            )
        else:
            new_data = base.data
